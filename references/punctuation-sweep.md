...
Three helper scripts live in `scripts/` (relative to the novel-skill repo root):
...
- `scripts/check_density.py` — detailed audit. Read-only. Reports word count, title validity, and a per-chapter density breakdown of every punctuation variant (each dash form, each ellipsis form, each quote form, Markdown marks, half-width punctuation residues).

All three scripts operate on `06-chapter-drafts/` (relative to the current working directory) by default. Pass a different folder as the first CLI argument to override. The user is expected to run the scripts from the novel project root (so that `06-chapter-drafts/` is the chapter folder of the project being worked on).
...
Out-of-range chapters are reported as `[HARD WARNING]` by both `check_dash.py` and `check_density.py`. `fix_dash.py` does **not** auto-fix word count — it must be done by editing the chapter (split or merge). The event is logged in `10-review/decisions-log.md` as a `chapter-length` row.
...
A missing or invalid title is reported as `[HARD ERROR]` by all three scripts. No exception is allowed — the title is mandatory. The scripts do not auto-fix it; the user must edit the chapter.
...
# 4. Detailed audit (optional but recommended for suspicious chapters)
python scripts/check_density.py
#    - per-punctuation-variant counts
#    - half-width punctuation residue detection
#    - read-only
...
# 6. Record the sweep stats in the micro-repair report
...
- Half-width punctuation inside Chinese sentences OTHER than quotes (e.g. `中文, 标点` — fix_dash only handles quote + comma pairs, not free-floating ASCII commas)
...
2. The constants (`SEMANTIC_REPLACE`, `MIN_CHARS`, `MAX_CHARS`, `BUDGET_*`) in `scripts/check_dash.py` (which `fix_dash.py` imports)
